...
def show_incubateurs(conn):
    ...
    print("-----Incubateurs-----")
    while raw:
        print(f"Nom: {raw[0]}, Création: {raw[1]}, Budget {raw[2]}")
        raw = cur.fetchone()
    print("-----------------------")
    cur.close()
...
def insert_project(conn):
    ...
    while True:
        ...
        type_projet = input(
            "Entrez le type du projet (Projet social : 1, Projet techno : 2, Projet artis : 3) : "
        )
        ...
        try:
            ...
            match int(type_projet):
                case 1:
                    cur.execute(
                        "INSERT INTO Projet_social (id_p, region) VALUES (%s, %s)",
                        (id, region),
                    )
                case 2:
                    cur.execute(
                        "INSERT INTO Projet_techno (id_p, innovation) VALUES (%s, %s)",
                        (id, innovation),
                    )
                case 3:
                    cur.execute(
                        "INSERT INTO Projet_artis (id_p, medium) VALUES (%s, %s)",
                        (id, medium),
                    )
...
def update_project(conn):
    ...
    while True:
        ...
        if id.lower() == "q":
            break
        choix = input(
            "Entrez la donnée à modifier (titre, descr, objectif, lancement, incubateur) ou 'q' : "
        )
        if choix.lower() == "q":
            break
        if choix not in {"titre", "descr", "objectif", "lancement", "incubateur"}:
            ...
            continue
        value = input(
            "Entrez la nouvelle valeur (appuyez sur entrée s'il y en a pas) : "
        )
        if value == "":
            value = None
...
def show_projects(conn):
    queries = [
        (
            "Projets Artistiques",
            "SELECT * FROM Projet JOIN Projet_artis ON id_p = id ORDER BY id",
        ),
        (
            "Projets Technologiques",
            "SELECT * FROM Projet JOIN Projet_techno ON id_p = id ORDER BY id",
        ),
        (
            "Projets Sociaux",
            "SELECT * FROM Projet JOIN Projet_social ON id_p = id ORDER BY id",
        ),
    ]
    cur = conn.cursor()
